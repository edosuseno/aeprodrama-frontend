...
import { useInfiniteShortwave2 } from "@/hooks/useShortwave2";
import { UnifiedMediaCard } from "./UnifiedMediaCard";
import { Loader2 } from "lucide-react";
import { UnifiedMediaCardSkeleton } from "./UnifiedMediaCardSkeleton";

export function ExploreShortwave2() {
    const {
        data: infiniteData,
        fetchNextPage,
        hasNextPage,
    ...
    } = useInfiniteShortwave2(undefined, []);
    ...
    if (isError && !infiniteData?.pages?.length) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-xl font-bold text-foreground mb-2">Menunggu Update Backend</h3>
                <p className="text-muted-foreground max-w-md">
                    Halaman antarmuka ini sudah siap! Namun API proxy (dracindo-backend) saat ini masih mengembalikan error. 
                    Silakan atur injeksi token/URL untuk shortwave2 di backend Anda agar kartu-kartu video muncul di sini.
                </p>
            </div>
        );
    }
                ...
                {infiniteData?.pages.map((page, pageIdx) =>
                    page.items.map((drama: any, idx: number) => (
                        <UnifiedMediaCard
                            ...
                            cover={drama.cover || drama.book_pic || drama.poster || drama.image || drama.cover_h || ''}
                            link={`/detail/shortwave2/${drama.id || drama.shortPlayId}?title=${encodeURIComponent(drama.title || drama.name)}&cover=${encodeURIComponent(drama.cover || drama.book_pic || drama.poster || drama.image || drama.cover_h || '')}`}
                            episodes={drama.totalEpisodes || drama.chapters || drama.chapterCount || 0}
                            index={idx}
                        />
                    ))
                )}
