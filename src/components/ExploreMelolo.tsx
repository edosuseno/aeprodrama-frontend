...
import { useInfiniteMelolo } from "@/hooks/useMelolo";
import { UnifiedMediaCard } from "./UnifiedMediaCard";
import { Loader2 } from "lucide-react";
import { UnifiedMediaCardSkeleton } from "./UnifiedMediaCardSkeleton";

export function ExploreMelolo() {
    const {
        data,
        fetchNextPage,
        hasNextPage,
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-9 gap-2 md:gap-3">
                {data?.pages.map((page, pageIdx) =>
                    page.map((drama: any, idx: number) => (
                        <UnifiedMediaCard
                            key={`${drama.book_id || drama.id}-${pageIdx}-${idx}`}
                            title={drama.book_name || drama.title || drama.name}
                            cover={drama.thumb_url || drama.cover || drama.book_cover}
                            link={`/detail/melolo/${drama.book_id || drama.id}`}
                            episodes={drama.serial_count || drama.episode_count || 0}
                            index={idx}
                        />
                    ))
                )}
