...
import { useInfiniteMeloShort } from "@/hooks/useMeloShort";
    ...
    if (allDramas.length === 0 && !isLoading) {
        return (
            <section className="text-center py-10">
                <p className="text-muted-foreground">Tidak ada drama ditemukan di MeloShort.</p>
            </section>
        )
    }
        ...
        <section className="space-y-4">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-9 gap-2 md:gap-3">
                {allDramas.map((drama, index) => (
                    <UnifiedMediaCard
                        ...
                        title={drama.title}
                        cover={drama.cover}
                        link={`/detail/meloshort/${drama.id}`}
                        episodes={drama.chapterCount}
                        onPrefetch={() => {
                            queryClient.prefetchQuery({
                                queryKey: ["meloshort", "detail", String(drama.id)],
                                queryFn: async () => {
                                    const res = await fetchJson<any>(`/api/meloshort/detail?id=${drama.id}`);
            ...
            <div ref={loadMoreRef} className="mt-4 py-8 flex flex-col justify-center items-center w-full min-h-[100px]">
                {isFetchingNextPage ? (
                    ...
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground opacity-50">Akhir dari Katalog MeloShort</p>
                )}
            </div>
        </section>
    );
