...
import { useInfiniteShortSky } from "@/hooks/useShortSky";
import { UnifiedMediaCard } from "./UnifiedMediaCard";
import { Loader2 } from "lucide-react";
import { UnifiedMediaCardSkeleton } from "./UnifiedMediaCardSkeleton";

export function ExploreShortSky() {
    const {
        data: infiniteData,
        fetchNextPage,
        hasNextPage,
        ...
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-muted-foreground max-w-md">
                    ...
                    Silakan atur injeksi token/URL untuk ShortSky di backend Anda agar kartu-kartu video muncul di sini.
                </p>
            </div>
        );
    }
                ...
                {infiniteData?.pages.map((page, pageIdx) =>
                    page.items.map((drama: any, idx: number) => (
                        <UnifiedMediaCard
                            ...
                            link={`/detail/shortsky/${drama.id || drama.shortPlayId}?title=${encodeURIComponent(drama.title || drama.name)}&cover=${encodeURIComponent(drama.cover || drama.book_pic || drama.poster || drama.image || drama.cover_h || '')}`}
