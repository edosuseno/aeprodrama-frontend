...
import { useInfiniteNetShort } from "@/hooks/useNetShort";
import { UnifiedMediaCard } from "./UnifiedMediaCard";
import { Loader2 } from "lucide-react";
import { UnifiedMediaCardSkeleton } from "./UnifiedMediaCardSkeleton";

export function ExploreNetShort() {
    const {
        data,
        fetchNextPage,
        hasNextPage,
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-9 gap-2 md:gap-3">
                {data?.pages.map((page, pageIdx) =>
                    page.map((drama: any, idx: number) => (
                        <UnifiedMediaCard
                            key={`${drama.shortPlayId || drama.id}-${pageIdx}-${idx}`}
                            title={drama.title || drama.shortPlayName}
                            cover={drama.cover || drama.shortPlayCover}
                            link={`/detail/netshort/${drama.shortPlayId || drama.id}`}
                            episodes={drama.totalEpisodes || drama.totalEpisode}
                            index={idx}
                        />
                    ))
                )}
