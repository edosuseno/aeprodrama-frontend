...
import { useDrmanovaDetail } from "@/hooks/useDrmanova";
...
export default function DrmanovaDetailPage() {
    const params = useParams<{ id: string }>();
    const id = params.id;
    const router = useRouter();

    const { data, isLoading, error } = useDrmanovaDetail(id || "");
    const { setPlatform } = usePlatform();

    const handleBack = () => {
        setPlatform("dramanova");
        router.push("/");
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
                        <div className="relative group">
                            ...
                            <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6">
                                <Link
                                    href={`/watch/dramanova/${detail.id}?ep=${detail.episodes?.[0]?.chapterIndex || 1}`}
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{detail.episodes?.length || 0} Episode</span>
                                    </div>
                            ...
                            <div className="glass rounded-xl p-4">
                                ...
                                <p className="text-muted-foreground leading-relaxed">
                                    {detail.introduction || detail.description || "Tidak ada deskripsi."}
                                </p>
                            </div>
