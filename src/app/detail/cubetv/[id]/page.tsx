...
export default function CubetvDetailPage() {
    const params = useParams<{ id: string }>();
    const searchParams = useSearchParams();
    const id = params?.id;
    const router = useRouter();

    const { data: detailData, isLoading, error } = useCubetvDetail(id || "");
    ...
    const detail = {
        ...actualData,
        title: actualData?.title && actualData.title !== 'CubeTV Drama' ? actualData.title : fallbackTitle,
        cover: actualData?.cover || actualData?.book_pic || actualData?.poster || fallbackCover,
        description: actualData?.description || actualData?.introduction || "Tidak ada deskripsi yang tersedia untuk drama ini."
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
                        <div className="relative group">
                            ...
                            <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6">
                                <Link
                                    href={`/watch/cubetv/${detail.id || id}?ep=1&title=${encodeURIComponent(detail.title)}&cover=${encodeURIComponent(detail.cover)}`}
