...
import { useDramaWaveDetail } from "@/hooks/useDramaWave";
...
export default function DramaWaveDetailPage() {
    const params = useParams<{ id: string }>();
    const id = params.id;
    const router = useRouter();

    const { data, isLoading, error } = useDramaWaveDetail(id || "");
    const { setPlatform } = usePlatform();

    const handleBack = () => {
        setPlatform("dramawave");
        router.push("/");
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
                        <div className="relative group">
                            ...
                            <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6">
                                <Link
                                    href={`/watch/dramawave/${detail.shortPlayId}?ep=1`}
