...
export default function StardustTVDetailPage() {
    const params = useParams<{ id: string }>();
    const id = params.id;
    const router = useRouter();

    const { data, isLoading, error } = useStardustTVDetail(id || "");
    const { setPlatform } = usePlatform();

    const handleBack = () => {
        setPlatform("stardusttv");
        router.push("/");
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
                        <div className="relative group">
                            ...
                            <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6">
                                <Link
                                    href={`/watch/stardusttv/${detail.id}?ep=1`}
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{detail.totalEpisodes || detail.chapterCount || detail.episodes?.length || 0} Episode</span>
                                    </div>
