...
export default function GoodShortDetailPage() {
    ...
    const { data: drama, isLoading, isError, refetch } = useGoodShortDetail(id || "");
    const { setPlatform } = usePlatform();

    const handleBack = () => {
        setPlatform("goodshort");
        router.push("/");
    };
    ...
    if (isError || !drama) {
        return (
            <div className="min-h-screen pt-24 px-4 bg-background">
                <UnifiedErrorDisplay
                    title="Gagal Memuat Detail GoodShort"
    ...
    const proxiedCover = getProxiedImage(drama.cover, 400);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
                        ...
                        <div className="relative group">
                            ...
                            <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-8">
                                <Link
                                    href={`/watch/goodshort/${drama.id}?ep=1`}
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                                    ...
                                    <span className="px-3 py-1 rounded-full border border-white/10 bg-white/5 text-primary text-[10px] font-bold uppercase tracking-wider">
                                        GoodShort Original
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="glass rounded-2xl p-6 border border-white/10">
                                ...
                                <p className="text-muted-foreground leading-relaxed text-sm md:text-base">
                                    {drama.description || "Nikmati drama pendek berkualitas tinggi dari platform GoodShort."}
                                </p>
                            </div>
