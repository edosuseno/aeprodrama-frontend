...
import { useFlexTVDetail, useFlexTVStream } from "@/hooks/useFlexTV";
...
export default function FlexTVWatchPage() {
    ...
    const { data: detailData, isLoading: loadingDetail, error: errorDetail } = useFlexTVDetail(id || "");
    const { data: videoUrl, isLoading: loadingStream } = useFlexTVStream(id || "", currentEpisode);
        ...
        if (detailData && currentEpisode < detailData.totalEpisodes) {
            ...
            if (id) {
                queryClient.prefetchQuery({
                    queryKey: ['flextv_watch', id, nextEp],
                    queryFn: async () => {
                        const res = await fetch(`/api/flextv/watch?id=${id}&episodeIndex=${nextEp}`);
    ...
    useEffect(() => {
        if (detailData && currentEpisode) {
            addToHistory({
                id: id || "",
                title: detailData.title || "FlexTV Drama",
                poster: detailData.cover || "",
                platform: "FlexTV",
                episodeNumber: currentEpisode,
                link: `/watch/flextv/${id}?ep=${currentEpisode}`
            });
        }
    ...
    const { processedVideoUrl, processedSubtitleUrl } = useMemo(() => {
        ...
        let pUrl = url;
        if (pUrl.startsWith("http") && !pUrl.includes("wolftv.online")) {
            pUrl = `/api/proxy?url=${encodeURIComponent(pUrl)}&referer=${encodeURIComponent('https://vidrama.asia/')}`;
        }
    ...
    useEffect(() => {
        if (processedVideoUrl && videoRef.current) {
            ...
            console.log(`[FlexTV] Playing: ${processedVideoUrl.substring(0, 50)}...`);
    ...
    const goToEpisode = (ep: number) => {
        ...
        router.replace(`/watch/flextv/${id}?ep=${ep}`, { scroll: false });
    ...
    const dramaTitle = detailData?.title || "FlexTV Drama";
