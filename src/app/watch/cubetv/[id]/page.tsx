...
import { useCubetvDetail } from "@/hooks/useCubetv";
...
export default function CubetvWatchPage() {
    ...
    const { data: detailData, isLoading: loadingDetail } = useCubetvDetail(id || "");
    const { addToHistory } = useHistoryStore();

    const actualData = detailData?.data || detailData;
    const episodes = actualData?.episodes || actualData?.list || [];
    ...
    let subtitleUrl = episodeData?.subtitle || '';

    // No need to prefix with /api/proxy because CubeTV returns valid M3U8 URLs directly,
    // but if it's blocked we can still use proxy. CubeTV usually works directly, but just in case:
    if (videoUrl && !videoUrl.includes('/api/proxy') && !videoUrl.includes('cubetv.cc')) {
        videoUrl = `/api/proxy?url=${encodeURIComponent(videoUrl)}`;
    }

    const handleVideoEnded = useCallback(() => {
        const actualDetail = detailData?.data || detailData;
        if (!actualDetail) return;
        const total = actualDetail.totalEpisodes || actualDetail.chapters || actualDetail.chapterCount || episodes.length;
        if (currentEpisode < total) {
            const nextEp = currentEpisode + 1;
            setCurrentEpisode(nextEp);
            window.history.replaceState(null, '', `/watch/cubetv/${id}?ep=${nextEp}`);
        }
    }, [currentEpisode, detailData, id, episodes.length]);

    const addLog = (msg: string) => {
        console.log(`[CubeTV Player] ${msg}`);
    };
    ...
    const goToEpisode = (ep: number) => {
        ...
        router.replace(`/watch/cubetv/${id}?ep=${ep}`, { scroll: false });
        setShowEpisodeList(false);
    };

    const fallbackTitle = searchParams?.get("title") || "CubeTV Drama";
    ...
    const dramaTitle = detailData?.title && detailData.title !== 'CubeTV Drama' ? detailData.title : fallbackTitle;
    ...
    useEffect(() => {
        if (detailData && currentEpisode) {
            addToHistory({
                ...
                link: `/watch/cubetv/${id}?ep=${currentEpisode}&title=${encodeURIComponent(dramaTitle)}&cover=${encodeURIComponent(fallbackCover)}`
            });
        }
