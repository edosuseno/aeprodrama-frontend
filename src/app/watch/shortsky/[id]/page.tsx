...
import { useShortSkyDetail, useShortSkyStream } from "@/hooks/useShortSky";
...
export default function shortskyWatchPage() {
    ...
    const { data: detailData, isLoading: loadingDetail } = useShortSkyDetail(id || "");
    const { addToHistory } = useHistoryStore();

    const { data: streamData, isLoading: loadingStream } = useShortSkyStream(id || "", currentEpisode);

    useEffect(() => {
        const episodes = detailData?.episodes || detailData?.list || [];
        const total = detailData?.totalEpisodes || detailData?.chapters || detailData?.chapterCount || episodes.length;
        if (currentEpisode < total && id) {
            queryClient.prefetchQuery({
                queryKey: ["ShortSky", "stream", id, currentEpisode + 1],
                queryFn: () => fetchJson<any>(`/api/shortsky/stream?shortPlayId=${id}&episodeNo=${currentEpisode + 1}`)
            });
        }
    }, [currentEpisode, id, detailData, queryClient]);

    const episodes = detailData?.episodes || detailData?.list || [];
    const episodeData = episodes.find((e: any) => e.episodeNo == currentEpisode || e.episodeNumber == currentEpisode || (e.index + 1) == currentEpisode);
    let videoUrl = streamData?.url || episodeData?._h264 || episodeData?._h265 || episodeData?.videoUrl || episodeData?.play_url;

    if (videoUrl && !videoUrl.includes('/api/proxy') && !videoUrl.includes('vidrama.asia')) {
    ...
    const handleVideoEnded = useCallback(() => {
        ...
        if (currentEpisode < total) {
            ...
            window.history.replaceState(null, '', `/watch/shortsky/${id}?ep=${nextEp}`);
        }
    }, [currentEpisode, detailData, id, episodes.length]);

    const addLog = (msg: string) => {
        console.log(`[shortsky Player] ${msg}`);
    };
    ...
    useEffect(() => {
        if (videoUrl && videoRef.current) {
            ...
            const isHlsUrl = videoUrl.includes('.m3u8') || videoUrl.includes('/proxy');
    ...
    const goToEpisode = (ep: number) => {
        ...
        router.replace(`/watch/shortsky/${id}?ep=${ep}`, { scroll: false });
    ...
    const totalEpisodes = detailData?.totalEpisodes || detailData?.chapters || detailData?.chapterCount || episodes.length || 0;
    const dramaTitle = detailData?.title && detailData.title !== 'ShortSky Drama' ? detailData.title : fallbackTitle;
    ...
    useEffect(() => {
        if (detailData && currentEpisode) {
            addToHistory({
                id: id || "",
                title: dramaTitle,
                poster: detailData.cover || detailData.cover_h || detailData.poster || fallbackCover,
                platform: "shortsky",
                episodeNumber: currentEpisode,
                link: `/watch/shortsky/${id}?ep=${currentEpisode}&title=${encodeURIComponent(dramaTitle)}&cover=${encodeURIComponent(fallbackCover)}`
            });
        }
    }, [id, currentEpisode, detailData, addToHistory, dramaTitle, fallbackCover]);

    // Swipe vertikal (mobile) & double-tap to zoom
    useEffect(() => {
        ...
        const handleTouchEnd = (e: TouchEvent) => {
            ...
            if (Math.abs(deltaY) > 80) {
                const totalEps = detailData?.totalEpisodes || detailData?.chapters || detailData?.episodes?.length || detailData?.chapterCount || 9999;
                ...
                <div className="relative w-full h-full flex items-center justify-center">
                    ...
                    {!videoUrl && !loadingDetail && !loadingStream && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 z-20">
                            ...
                            <p className="text-white mb-4">Video tidak tersedia (Menunggu API Backend atau VIP)</p>
                            <button
