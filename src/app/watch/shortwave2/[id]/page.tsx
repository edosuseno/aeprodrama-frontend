...
export default function Shortwave2WatchPage() {
    ...
    const { data: detailData, isLoading: loadingDetail } = useShortwave2Detail(id || "");
    const { addToHistory } = useHistoryStore();

    // Moved below to use dramaTitle
    ...
    const videoUrl = episodeData?._h264 || episodeData?._h265 || episodeData?.videoUrl || episodeData?.play_url;
    ...
    const handleVideoEnded = useCallback(() => {
        ...
        if (currentEpisode < total) {
            ...
            window.history.replaceState(null, '', `/watch/shortwave2/${id}?ep=${nextEp}`);
        }
    }, [currentEpisode, detailData, id, episodes.length]);

    const addLog = (msg: string) => {
        console.log(`[Shortwave2 Player] ${msg}`);
    };
    ...
    const goToEpisode = (ep: number) => {
        ...
        router.replace(`/watch/shortwave2/${id}?ep=${ep}`, { scroll: false });
        setShowEpisodeList(false);
    };

    const fallbackTitle = searchParams.get("title") || "Shortwave2 Drama";
    ...
    const dramaTitle = detailData?.title && detailData.title !== 'Shortwave2 Drama' ? detailData.title : fallbackTitle;
    ...
    useEffect(() => {
        if (detailData && currentEpisode) {
            addToHistory({
                ...
                link: `/watch/shortwave2/${id}?ep=${currentEpisode}&title=${encodeURIComponent(dramaTitle)}&cover=${encodeURIComponent(fallbackCover)}`
            });
        }
