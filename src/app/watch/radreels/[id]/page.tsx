...
export default function RadreelsWatchPage() {
    ...
    const { data: detailData, isLoading: loadingDetail, error: errorDetail } = useRadreelsDetail(id || "");
    ...
    useEffect(() => {
        if (detailData && currentEpisode) {
            addToHistory({
                id: id || "",
                title: detailData.title || "Radreels Drama",
                poster: detailData.cover || "",
                platform: "radreels",
                episodeNumber: currentEpisode,
                link: `/watch/radreels/${id}?ep=${currentEpisode}`
            });
        }
    ...
    useEffect(() => {
        if (processedVideoUrl && videoRef.current) {
            ...
            console.log(`[Radreels] Playing: ${processedVideoUrl.substring(0, 50)}...`);
    ...
    const goToEpisode = (ep: number) => {
        ...
        router.replace(`/watch/radreels/${id}?ep=${ep}`, { scroll: false });
    ...
    const dramaTitle = detailData?.title || "Radreels Drama";
