...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Concept derived from stardusttv proxy but mapped to shortsky upstream
    const response = await fetch(`${backendUrl}/shortsky/detail?shortPlayId=${shortPlayId}`, {
      cache: 'no-store'
    });
    ...
    console.error("shortsky Proxy Error:", error);
